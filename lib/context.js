...
class ContextSession {
  ...
  valid(value, key) {
    ...
    if (value.__expire && value.__expire < Date.now()) {
  ...
  async commit() {
    ...
    const reason = this.__shouldSaveSession();
  ...
  __shouldSaveSession() {
    ...
    // force save session when `session.__requireSave` set
    if (session.__requireSave) return 'force';
  ...
  async save(changed) {
    ...
    if (maxAge === 'session') {
      ...
      // set expire for check
      json.__expire = maxAge + Date.now();
      json.__maxAge = maxAge;
    }
