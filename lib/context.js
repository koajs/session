...
class ContextSession {
  ...
  async initFromExternal() {
    ...
    debug('get external key from cookie %s', cookie);
    ...
    let externalKey = cookie;
    try {
      if (typeof opts.decode === 'function') {
        externalKey = opts.decode(externalKey);
      }
    } catch (err) {
      // create a new `externalKey`
      this.create();
  ...
  initFromCookie() {
    ...
    try {
      json = typeof opts.decode === 'function' ? opts.decode(cookie) : util.decode(cookie);
    } catch (err) {
  ...
  async save(changed) {
    ...
    if (externalKey) {
      ...
      let cookie = externalKey;
      if (typeof opts.encode === 'function') {
        cookie = opts.encode(externalKey);
      }
      this.ctx.cookies.set(key, cookie, opts);
      return;
    }

    // save to cookie
    debug('save %j to cookie', json);
    json = typeof opts.encode === 'function' ? opts.encode(json) : util.encode(json);
    debug('save %s', json);
