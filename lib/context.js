...
class ContextSession {
  ...
  constructor(ctx, opts) {
    ...
    this.external = this.opts.external;
  }
  ...
  * initFromExternal() {
    ...
    const json = yield this.external.get(externalKey, ctx);
    if (!this.valid(json)) {
      this.createSession();
      return;
    }
    ...
    this.createSession(json, externalKey);
  ...
  createSession(val, externalKey) {
    ...
    if (this.opts.external) this.externalKey = externalKey || uid.sync(24);
  ...
  * removeSession() {
    ...
    if (externalKey) yield this.external.remove(externalKey);
  ...
  * saveSession() {
    ...
    if (externalKey) {
      ...
      yield this.external.set(externalKey, json, maxAge);
